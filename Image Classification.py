...
Original file is located at
    https://colab.research.google.com/drive/1auSthBp1JTYO4BUG-TRPZtqo2hPDuJwD
"""
...
def myGetModel(obj):
    ...
    inputDim = obj.input_dim
    num_classes = obj.num_classes
    weight_decay = 1e-4
    
    model = Sequential()
    
    model.add(Conv2D(32, (3, 3), padding='same', input_shape=(inputDim),kernel_regularizer=regularizers.l2(weight_decay)))
...
def myFitModel(model,obj):
    ES = EarlyStopping(monitor='val_accuracy', mode='min', patience = 10)
    MC = ModelCheckpoint('/temp/weights-best.hdf5', monitor='val_accuracy', mode='max', save_best_only=True)
    model.fit(obj.x_train, obj.y_train, epochs=100, batch_size=64, validation_data=(obj.x_valid,obj.y_valid),callbacks=[ES,MC])
    model.load_weights('/temp/weights-best.hdf5')
    return model
